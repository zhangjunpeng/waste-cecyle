...
class MainViewModel(val dataSource: DataSource)  : BaseViewModel() {
    ...
    fun memberShowInfo(){
        viewModelScope.launch {
            return@launch withContext(Dispatchers.IO){
                val result=dataSource.memberShowInfo(SPUtils.getInstance().getString(NameSpace.TokenName),"1")
    ...
    fun cartTotal(){
        viewModelScope.launch {
            return@launch withContext(Dispatchers.IO){
                val result=dataSource.carTotal(SPUtils.getInstance().getString(NameSpace.TokenName),"1")
                if (result is RequestResult.Success){
                    memberShowInfonResult.postValue(result.data)
                }else if (result is RequestResult.Error){
                    errorResult.postValue(result.error)
